...
class FallDetector:
    ...
    def get_features(self, end_idx):
        ...
        idx = end_idx - self.config["sliding_window_length"] + 1
        x_frames = []
        y_frames = []
        ...
        for i in range(self.config["sliding_window_length"]):
            flow_x_file = self.config["flow_path"] + "flow_x/flow_img_" + str(idx)
            flow_y_file = self.config["flow_path"] + "flow_y/flow_img_" + str(idx)
